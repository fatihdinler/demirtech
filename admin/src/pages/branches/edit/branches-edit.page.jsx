...
const BranchesEdit = () => {
  const {
    name, customerId, address, contactInfo, onChange, editBranch, clearPageHandler,
    customersOptions, handleCustomersChange, userIds, usersOptions, handleUsersChange,
  } = useBranchesEdit()

  return (
    <div>
      <Breadcrumb paths={[{ label: 'Şubeler', link: '/branches' }, { label: 'Düzenle' }]} />
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        ...
        <PageFooter createOrEditHandler={editBranch} cancelHander={clearPageHandler} />
      </div>
    </div>
  )
}
