...
const LocationsCreate = () => {
  const {
    ...
    customerId, customersOptions, handleCustomersChange, clearPageHandler, createLocation,
  } = useLocationsCreate()

  return (
    <div>
      <Breadcrumb paths={[{ label: 'Lokasyonlar', link: '/locations' }, { label: 'Oluştur' }]} />
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 max-w-3xl">
        ...
        <PageFooter isCreatePage createOrEditHandler={createLocation} cancelHander={clearPageHandler} />
      </div>
    </div>
  )
}
